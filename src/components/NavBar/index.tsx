...
const NavLink: React.FC<NavLinkParams> = ({ to, children }) => {
  ...
  return (
    <Link
      href={to}
      locale={locale}
      className={`mx-5 text-2xl font-bold hover:text-primary ${
...
const MobileNav: React.FC = () => {
  ...
  return (
    <nav>
      ...
      <section
        className={`fixed w-full h-full top-0 right-0 left-0 bg-light-color dark:bg-main-color z-10 transform ease-in-out duration-300 ${
          open ? "translate-x-0" : "translate-x-full"
        } `}
      >
        <section className="flex items-center justify-center h-20 bg-light-color dark:bg-main-color filter drop-shadow-md">
          <Logo />
        </section>
...
const NavBar: React.FC = () => {
  ...
  return isMobile ? <MobileNav /> : <NavigationBar />;
};
